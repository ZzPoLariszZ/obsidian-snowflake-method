...
- **时间线**是可视化工作区中做好的第二个视图，关注的不是场景的阅读顺序，而是它们发生的时间。每条**时间线**自成一列，有自己的名称，也可以绑定到某个角色或世界观笔记。**视图**决定显示哪几条时间线以及它们的排列顺序，一个项目可以建立任意多个视图。工具栏上的**添加时间线**与**添加视图**分别用来新建两者，视图的表单可以重命名视图、用手柄调整其中时间线的顺序、添加或移除时间线，也可以删除视图本身。命令面板中的**打开时间线工作区**可以直接打开这个标签页。
- 左侧是所有时间线**共用的时间列**，内容取自项目中已有的时间笔记，主描述保存在笔记里，不会复制到时间线中。每条时间线都与这些时间对齐在同一行上，因此可以直接横向比较，轴线上的节点标出每条时间线实际涉及的时间。时间可以从时间线的表头添加，也可以点击空单元格中的加号添加，还可以插入到某个时间之前或之后，用拖动或菜单调整顺序，移除时其中的子描述会一并移除。点击描述会打开该时间笔记的表单并定位到描述字段，笔记已被删除的时间显示为「时间笔记已缺失」，只能移除。
- **子描述**在单元格中逐行书写，记录某条时间线在某个时间发生了什么。在末尾的输入框中输入并确认即可添加一行，焦点会留在原处以便继续写下一行，已有的行可以就地编辑，行的菜单可以上移、下移、移动到其他时间，其中已放入场景时则需确认后才能移除。写入失败时本会丢失的文字会保留下来：该行仍在时保留到下次编辑，该行已不在时则显示出来供你自行复制。
...
- 时间线**保存在一个文件中**，位于项目的可视化文件夹下，其中只记录标识，也就是场景与实体的 id，不记录它们的名称与描述，这些内容仍保存在各自的笔记里。该文件的读取规则与任务、伏笔、修订的文件一致：无法读取的条目会被保留在一旁而不是丢弃，由更新版本插件写入的文件会原样保持不变，已损坏的文件会另存备份，并重新开始记录。

### 变更

- 工作区的标签条现在依次是**场景看板**、**自由画布**、**时间线**与**节拍表**。时间线做好之后排到了节拍表之前，**情节线**已从标签条中移除。
- 每张场景卡片的顶部中央都有一个**拖动握柄**，指针移到卡片上或正在拖动卡片时显示，卡片不可拖动时不显示。场景看板中的卡片也是如此，因为场景看板、时间线的各列与场景池如今共用同一种卡片。

### 修复

- 插件正在卸载时，如果场景的文字写入失败，这些文字会输出到控制台，而不是弹出一个比工作区本身存在更久的对话框。仅仅关闭工作区时，仍会像以前一样弹出对话框。
