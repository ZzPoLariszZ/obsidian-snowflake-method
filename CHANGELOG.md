...
- A cell's foot waits to be asked for. It used to stand in sight wherever no row was above it, which is one invitation beside a lone timeline and the same invitation repeated through every empty cell once timelines stand side by side. Every foot now keeps to itself until the pointer or the keyboard is in its own cell, or a drag is under way, since a row or a scene can be dropped on one, as a foot under rows already did. The wording still follows the rows standing above it, and on a touch screen every foot stays in sight, as the other controls there do.
- The scene pool's mark for a scene coming back is a rounded dashed frame with room of its own, over a faint wash, in the accent the lanes and their cells already wear. It was an inset shadow, square at the corners and painted under the cards, and the cards stand against the board's sides, so down each side the frame showed only in the gaps between them. The pool is wider by the room the frame draws in, so a card is as wide as it was, and the workspace folds the time column and the pool by itself at a window one rem wider than before.
...
- 单元格底部用于添加子描述的输入框，如今只在指针停在该单元格上、或焦点落入该单元格时才显示，拖动进行期间也会显示，因为子描述行与场景都可以放到它上面。此前只要单元格里还没有子描述，这个输入框就一直显示着。只有一条时间线时，这句提示是有用的，但多条时间线并排时，每个空单元格都会显示同一句提示，反而成了干扰。如今不论上方有没有子描述，显示规则都一样，输入框中的提示文字仍按上方是否已有子描述而变化。触摸屏上这些输入框始终显示，与那里的其他控件一致。
- 把场景拖回场景池时的高亮框，如今是带圆角的虚线框，四周留出了空隙，并有一层同色的淡淡底色，用的是时间线各列和单元格已有的强调色。此前它是直角的内阴影，而且被卡片遮住，因为卡片正好紧贴面板两侧，所以左右两边只能从卡片之间的缝隙中看到它。场景池相应加宽，加宽的幅度正是让出的这部分空隙，因此卡片宽度不变。场景池变宽之后，工作区自动收起时间列与场景池的窗口宽度也相应提高了 1rem。
- 时间线各列之间的间距改用 gap 简写属性书写，不再单独使用 column-gap，后者会被社区审核的浏览器兼容性检查判定为多栏布局。两种写法设定的间距完全相同，行与行之间原本也没有间距，因此显示效果没有任何变化。

## [0.20.0]

### 新增

- **时间线**是可视化工作区中做好的第二个视图，关注的不是场景的阅读顺序，而是它们发生的时间。每条**时间线**自成一列，有自己的名称，也可以绑定到某个角色或世界观笔记。**视图**决定显示哪几条时间线以及它们的排列顺序，一个项目可以建立任意多个视图。工具栏上的**添加时间线**与**添加视图**分别用来添加时间线和视图，视图的表单可以重命名视图、用拖动手柄调整其中时间线的顺序、添加或移除时间线，也可以删除视图本身。命令面板中的**打开时间线工作区**可以直接打开这个标签页。
- 左侧是所有时间线**共用的时间列**，内容取自项目中已有的时间笔记，主描述保存在笔记里，不会复制到时间线中。每条时间线都与这些时间在同一行上对齐，因此可以直接横向比较，轴线上的节点标出每条时间线实际涉及的时间。时间可以从时间线的表头添加，也可以点击空单元格中的加号添加，还可以插入到某个时间之前或之后，用拖动或菜单调整顺序。移除某个时间时，它在这条时间线上的子描述会一并移除。点击描述会打开该时间笔记的表单并定位到描述字段，笔记已被删除的时间显示为「时间笔记已缺失」，只能移除。
- **子描述**在单元格中逐行书写，记录某条时间线在某个时间发生了什么。在末尾的输入框中输入并确认即可添加一行，焦点会留在原处以便继续写下一行，已有的行可以就地编辑，行的菜单可以上移、下移、移动到其他时间，也可以移除。若行中已放入场景，移除前会再确认一次。子描述无法写入时，原本会丢失的文字会保留下来：该行仍在时保留到下次编辑，该行已不在时则显示出来供你自行复制。
- 右侧的**场景池**是单列形式的场景看板，其中是当前时间线尚未放置的全部场景，搜索、筛选、显示与排序都与看板相同。把卡片从池中拖到某条子描述上即可放置，拖回池中即可收回。一个场景在同一条时间线上只占一个位置，因此再次放置是移动而不是复制。这些操作都不必依赖拖动，**移动到子描述**、**移动到时间**、**移动到位置**等菜单项可以完成同样的事。
- 场景的展示方式可以按视图设为**平铺**或**堆叠**，平铺把每张卡片依次排开，堆叠只显示最前面的一张，并配有计数与左右箭头供逐张查看。视图中只有一条时间线时默认平铺，有多条时默认堆叠，手动设置之后以设置为准。工具栏上还可以隐藏子描述以便只看卡片、让最晚的时间排在最前，以及收起时间列与场景池，窗口宽度不足以容纳两者时会自动收起。
- 时间线**保存在一个文件中**，位于项目的可视化文件夹下，其中只记录标识，也就是场景与实体的 id，不记录它们的名称与描述，这些内容仍保存在各自的笔记里。该文件的读取规则与任务、伏笔、修订的文件一致：无法读取的条目会原样保留，而不是丢弃，由更新版本的插件写入的文件会保持原样，已损坏的文件会另存备份，并重新开始记录。

### 变更

- 工作区的标签条现在依次是**场景看板**、**自由画布**、**时间线**与**节拍表**。时间线做好之后排到了节拍表之前，**情节线**已从标签条中移除。
- 每张场景卡片的顶部中央都有一个**拖动手柄**，指针移到卡片上或正在拖动卡片时显示，卡片不可拖动时不显示。场景看板中的卡片也是如此，因为场景看板、时间线的各列与场景池如今共用同一种卡片。

### 修复

- 插件正在卸载时，如果场景的文字写入失败，这些文字会输出到控制台，而不是弹出一个在工作区关闭后仍留在屏幕上的对话框。仅仅关闭工作区时，仍会像以前一样弹出对话框。
- 未保存的文字不会再被写回已经不存在的卡片。写入过程中被移除的卡片，以及所属场景已被删除的卡片，都会被跳过，而不会被当作当初正在编辑的那张卡片。
