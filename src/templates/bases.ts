...
/**
 * The spellings a 0.7.0 base carried for its role column and role sheets,
 * mapped onto today's. A base written before roles became category links
 * filters on the legacy type key alone -- the very key the migration
 * deletes -- so without this swap its role views would empty out the moment
 * the notes crossed. The legacy halves are still generated by the same
 * functions (the type formula survives as the fallback inside the role
 * formula), so the two sides of a swap cannot drift apart. Every swap is
 * anchored to whole lines: the legacy formula is a substring of today's,
 * and matched bare it would nest the formula into itself on a base that has
 * already crossed.
 */
export function getLegacyRoleRefreshes(
  ...
  const refreshes: BaseTextRefresh[] = [
    {
      from: `\n  character_type: ${characterTypeFormula(copy)}\n`,
      to: `\n  character_type: ${characterRoleFormula(copy, roles)}\n`,
    },
  ];
  for (const type of ["major", "supporting", "minor"] as const) {
    const legacyFilter = scalar(equals(key, type));
    refreshes.push({
      from: `\n        - ${legacyFilter}\n`,
      to:
        `\n        - or:\n` +
        `            - ${scalar(categoryContains(roles[type]))}\n` +
        `            - ${legacyFilter}\n`,
    });
  }
  return refreshes;
